...
public final class ScreenshotEditorFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        final int highlightColor = ContextCompat.getColor(getActivity(), R.color.feedback_highlight_transparent_semi);
        final int blackoutColor = ContextCompat.getColor(getActivity(), R.color.feedback_black);
        final Uri fileUri = getArguments().getParcelable(ARG_SCREENSHOT_URI);
        final View view = inflater.inflate(R.layout.feedback_screenshot_preview, container, false);
        final Listener presenter = (Listener) getActivity();

        Toolbar toolbar = (Toolbar) view.findViewById(R.id.toolbar);
        toolbar.setNavigationIcon(R.drawable.feedback_ic_close_black_24dp);
        ...
        toolbar.inflateMenu(R.menu.feedback_menu_screenshot_editor);
        toolbar.setOnMenuItemClickListener(new Toolbar.OnMenuItemClickListener() {
            @Override
            public boolean onMenuItemClick(MenuItem item) {
                if (item.getItemId() == R.id.feedback_action_save) {
                    ViewUtils.exportViewToFile(view.findViewById(R.id.feedback_screenshot_preview_image_view_updated), new File(fileUri.getPath()));
                    presenter.onDoneWithScreenshotEditing();
                    return true;
                } else {
                    return false;
        ...
        ImageView imageView = (ImageView) view.findViewById(R.id.feedback_screenshot_preview_image);
        imageView.setImageURI(fileUri);

        final DrawableView drawableView = (DrawableView) view.findViewById(R.id.feedback_screenshot_preview_image_drawable_view);
        ...
        colorChooser.setOnCheckedChangeListener(new RadioGroup.OnCheckedChangeListener() {
            ...
            public void onCheckedChanged(RadioGroup group, int checkedId) {
                if (checkedId == R.id.feedback_screenshot_preview_pick_highlight_color) {
                    config.setStrokeColor(highlightColor);
                } else {
        ...
        view.findViewById(R.id.feedback_screenshot_preview_undo).setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                drawableView.undo();
            }
